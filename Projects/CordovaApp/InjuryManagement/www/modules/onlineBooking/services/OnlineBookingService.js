 ...
 */
angular.module('starter.booking.services', []).factory('OnlineBookingService', function(Restangular) {
    var bookingService = {};
    var selectedBooking = {};
    ...
    bookingService.getLocationsFilter = function(date) {
        var getLF = api.all('rlob/redimedsites/list-mobile');
        return getLF.post({
            date: date
        });
    }
    ...
    bookingService.getDoctorsFilter = function(date) {
        var getDF = api.all('rlob/doctors/get-doctors-for-source-type-mobile');
        return getDF.post({
            date: date
        });
    }
    bookingService.getApointmentCalendar = function(specialityId, doctorId, locationId, fromTime) {
        var getAC = api.one('rlob/appointment-calendar/get-appointment-calendar-not-service');
        return getAC.get({
            DOCTOR_ID: doctorId,
            SITE_ID: locationId,
            FROM_TIME: fromTime
