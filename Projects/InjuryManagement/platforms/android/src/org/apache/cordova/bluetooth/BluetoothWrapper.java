...
public class BluetoothWrapper
{
    ...
    /*
    to control the list of device that has in discovery state
    Prevent from sending the device 2 or 3 times to the Cordova
    PhuongNM
    */
    private ArrayList _deviceList;
    public String deviceType = "";
    public String deviceName = "";
    public boolean isDiscoveryOneDevice = false;
	/**
	...
	public boolean isDiscoveryOneDevice() throws Exception
	{
		try
		{
		    isDiscoveryOneDevice = true;
			return isDiscoveryOneDevice;
		}
		catch(Exception e)
		{
			throw e;
	...
	{
	    //clear all device in device list before discovery. PhuongNM
	    _deviceList.clear();
		try
		{
			...
			else if(BluetoothDevice.ACTION_FOUND.equals(action))
			{
				try
				{
					...
					if(!_deviceList.contains(device.getName()) && device.getName() != null && !deviceName.contains("Unknown") ){
                        ...
                        msg.setData(bundle);
                        msg.sendToTarget();
                        //PhuongNM: only discover 1 device and stop
                        if(isDiscoveryOneDevice){
                            stopDiscovery();
                        }
					}
