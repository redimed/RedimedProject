...
.controller('rlobNonEmergencyController',function($scope, $state, SecurityService,$ionicLoading,$ionicPopup,$filter,$cordovaDatePicker,$ionicModal,$ionicPopup,$timeout,$filter, $cordovaDatePicker,$cordovaKeyboard,$cordovaAppVersion,localStorageService){
    $ionicLoading.hide();
    ...
    if(localStorageService.get("NONEMERGENCY") !== null){
                    ...
                    $scope.non.date= $scope.data.date;
                    $scope.non.month= $scope.data.month;
                    $scope.non.year= $scope.data.year;
        ...
        $scope.booking = function(){
             if($scope.non.REMEMBER_PATIENTS == 1){
                ...
                console.log( localStorageService.get("NONEMERGENCY"))
            }else
            {
