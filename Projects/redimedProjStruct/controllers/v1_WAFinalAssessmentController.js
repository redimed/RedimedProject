...
module.exports = {
    ...
    postInsertAs: function(req, res){
        var wafinal = req.body;
        console.log(req.body);

        var sql = model_sql.wa_new_id();
        var k_sql = res.locals.k_sql;

        k_sql.exec(sql,function(data){
            var query = squel.insert()
                .into('th_final_assessment')
                .set('AssessmentName','Progress Assessment')
                .set('reportlocal','WA');

            for(var key in wafinal){
                if(wafinal[key]){
                    query.set(key ,wafinal[key]);
                }
            }
    ...
    postEditAs: function(req, res){
        ...
        var query = squel.update()
            .table("th_final_assessment");
        for(var key in wafinal){
            query.set(key ,wafinal[key]);
        }
        query.where("id = ?", wafinal.id);
        k_sql.exec(query.toString(),function(){
            console.log("Edit final data successfully!");
            res.end();
        },function(err){
            console.log(err);
            res.end();
