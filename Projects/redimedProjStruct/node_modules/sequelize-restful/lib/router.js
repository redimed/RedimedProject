  ...
  switch(match.length) {
    ...
    case 2: // requested path: /api/dao_factory/1
      ...
      */
        ...
        switch(req.method) {
          case 'GET':
            handleResourceSearch.call(this, modelName, req, callback)
            break;
          case 'POST':
            break;
          default:
...
Router.prototype.findDAOFactory = function(modelName) {
  return this.sequelize.daoFactoryManager.getDAO(modelName, { attribute: 'name' })
...
Router.prototype.findAssociation = function(daoFactory, associatedModelName) {
  ...
  for (var key in  daoFactory.associations){
    if(key.toLowerCase() === associatedModelName.toLowerCase()){
...
var handleResourceShow = function(modelName, identifier, callback) {
  ...
  if (!!daoFactory) {
    // KHANK EDIT
    var key = daoFactory.primaryKeyAttributes[0];
    var search = {};
...
*   KHANK ADDED
*/
var handleResourceSearch = function(modelName, req, callback) {
  ...
  switch(req.method) {
    case 'POST':
      key = 'body';
      fields = req.body.fields ? req.body.fields : null;
      where_opt = req.body.where ? req.body.where : null;
      break;
    case 'GET':
      if(req.query.fields) {
  ...
  var limit = (req[key].limit) ? req[key].limit: 10;
  var offset = (req[key].offset) ? req[key].offset: 0;
  var order = (req[key].order) ? req[key].order: null;

  daoFactory.findAndCountAll({
      where: where_opt,
      offset: offset,
      limit: limit,
      attributes: fields,
