angular.module('app.loggedIn.template.directives.patient_edit', [])

.directive('templatePatientEdit', function($modal, $cookieStore, $state, $stateParams, TemplateModel, toastr, PatientService){
	return {
		restrict: 'EA',
		scope: {
			id: '=',
		...
		templateUrl: 'modules/template/directives/templates/patient_edit.html',
		link: function(scope, elem, attrs){
			...
			scope.save = function(){
				...
				var postData = {name: scope.form.name, content: html, id: scope.id};
			...
			var loadTemplate = function(){
				TemplateModel.one({id: scope.id})
				.then(function(response){
					scope.form = response.data[0];

					element.wysiwyg('shell').setHTML(scope.form.content);
				})
			}
