...
//    Metronic.init();
        ...
        var datePaginator=new MyDatePaginator('#mydatepaginator',$scope.selectedInfo.var1,datePaginatorChanged);
...
//        $scope.updateAppoinmentsList();
        ...
        $scope.dialogStyleClass='test-dialog';
        $scope.dialogTest=function()
        {
            $(".test-dialog").modal({show:true,backdrop:'static'})
        }
        $scope.testAppointmentCalendar={};
        $scope.$watch("testAppointmentCalendar", function(newValue, oldValue){
//            if($scope.testAppointmentCalendar)
//                alert(JSON.stringify($scope.testAppointmentCalendar));
        });
