...
.controller("rlob_admin_bookingListController", function($scope, $http,$window,rlTypesService,doctorsService,locationService,FileUploader,$cookieStore) {
    $scope.status=false;
...
//    }];
    ...
    uploader.onSuccessItem = function(fileItem, response, status, headers) {
        alert(JSON.stringify(response.fileInfo));
...
//                    uploader.formData[0].push({booking_id:$scope.selectedBooking.BOOKING_ID,company_id:$scope.loginInfo.company_id,worker_name:$scope.selectedBooking.WRK_SURNAME});
    ...
    $scope.lob_change_status=function(bookingId,status,scope)
    {
        $scope.msgDialog={};
        $http({
            method:"POST",
            ...
            .success(function(data) {
                if(data.status=='success')
                {
                    ...
                    $scope.msgDialog.status="success";
                    $scope.msgDialog.msg='Changing status to ['+status+'] success!';


                }
                else
                {
                    $scope.msgDialog.status="fail";
                    $scope.msgDialog.msg='Changing status to ['+status+'] fail!';
                }
            })
            .error(function (data) {
                $scope.msgDialog.status="fail";
            ...
            .finally(function() {
                $scope.msgDialog.header="Change Booking Status";
                $("#lob-msg-dialog").modal({show:true,backdrop:'static'});
            });
    }
        ...
        $http({
            ...
            .success(function(data) {
                if(data.status=='success')
                {
                    ...
                    $scope.msgDialog.status="success";
                    $scope.msgDialog.msg='Changing success! Customer'+(role==1?' can ':' cannot ')+'download this file';
                }
                else
                {
                    $scope.msgDialog.status="fail";
                    $scope.msgDialog.msg='Changing fail!';
                }
            })
            .error(function (data) {
                $scope.msgDialog.status="fail";
            ...
            .finally(function() {
                $scope.msgDialog.header="authority downloads";
                $("#lob-msg-dialog").modal({show:true,backdrop:'static'});
            });
    }
});
